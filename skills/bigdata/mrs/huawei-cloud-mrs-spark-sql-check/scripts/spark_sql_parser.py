# -*- coding: utf-8 -*-
"""
MRS Spark SQL Parser

Recursive descent parser for MRS Spark SQL statements.
...
from spark_sql_tokenizer import Token, TokenType, tokenize
...
class SparkSQLParser:
    """
    MRS Spark SQL Recursive Descent Parser

    Parses token stream into AST following the Spark SQL grammar rules.
    Supports Spark-specific syntax including USING, OPTIONS, PARTITIONED BY,
    CACHE TABLE, UNCACHE TABLE, REFRESH, ADD/LIST JAR, etc.
    """
    ...
    def parse(self):
        ...
        elif stmt_type in ("CREATE VIEW", "CREATE OR REPLACE TEMP VIEW",
                           "CREATE GLOBAL TEMP VIEW", "CREATE MATERIALIZED VIEW"):
        ...
        elif stmt_type == "CACHE TABLE":
            ast = self._parse_cache_table()
        elif stmt_type == "UNCACHE TABLE":
            ast = self._parse_uncache_table()
        elif stmt_type == "CLEAR CACHE":
            ast = self._parse_clear_cache()
        elif stmt_type in ("REFRESH", "REFRESH TABLE", "REFRESH FUNCTION"):
            ast = self._parse_refresh()
        elif stmt_type == "ADD JAR":
            ast = self._parse_add_jar()
        elif stmt_type == "LIST JAR":
            ast = self._parse_list_jar()
        else:
    ...
    def _parse_create_table(self):
        ...
        while not self._at_end():
            ...
            if t.type == TokenType.KEYWORD and t.value.upper() in (
                ...
                "USING", "OPTIONS", "LOCATION", "TBLPROPERTIES", "AS", "LIKE",
                "SKEWED", "CONSTRAINT"):
                break
        ...
        # Scan remaining tokens for Spark/Hive extensions
        while not self._at_end():
            token = self._current()
            ...
            # ROW FORMAT
                ...
                if self._current().is_keyword("FORMAT"):
                    self._advance()
                rf_tokens = self._collect_until("STORED", "USING", "OPTIONS",
                                                "LOCATION", "TBLPROPERTIES",
                                                "COMMENT", "AS")
                node.children["row_format"] = self._tokens_to_text(rf_tokens).strip()

            # USING (Spark DataSource API)
            elif token.is_keyword("USING"):
                self._advance()
                # Data source name (keyword or identifier or string)
                if self._current().type == TokenType.KEYWORD:
                    node.children["using"] = self._advance().value.upper()
                elif self._current().type in (TokenType.IDENT, TokenType.BACKTICK_IDENT):
                    node.children["using"] = self._advance().value
                elif self._current().type == TokenType.SCONST:
                    node.children["using"] = self._advance().value.strip("'\"")

            # OPTIONS (Spark data source options)
            elif token.is_keyword("OPTIONS"):
                self._advance()
                if self._current().type == TokenType.LPAREN:
                    opt_tokens = self._collect_balanced_parens()
                    node.children["options"] = self._tokens_to_text(opt_tokens).strip()
            ...
            # LIKE
                ...
                self._advance()
                like_tokens = self._collect_until("STORED", "USING", "OPTIONS",
                                                  "LOCATION", "TBLPROPERTIES",
                                                  "COMMENT", "ROW")
    ...
    def _parse_create_view(self):
        ...
        if self._current().is_keyword("GLOBAL"):
            self._advance()
            node.children["is_global"] = True
        ...
        if self._current().is_keyword("MATERIALIZED"):
            self._advance()
            node.children["is_materialized"] = True
        if self._current().is_keyword("VIEW"):
            self._advance()
        ...
        node.children["as_select"] = self._tokens_to_text(remaining).strip()
    ...
    def _parse_cache_table(self):
        node = ASTNode("CacheStmt")
        start_token = self._current()

        self._advance()  # skip CACHE

        # LAZY
        if self._current().is_keyword("LAZY"):
            self._advance()
        ...
        while not self._at_end():
            remaining.append(self._advance())
        node.children["table"] = self._tokens_to_text(remaining).strip()
    ...
    # ============================================================
    # UNCACHE TABLE Parser
    # ============================================================

    def _parse_uncache_table(self):
        node = ASTNode("UncacheStmt")
        start_token = self._current()

        self._advance()  # skip UNCACHE
    ...
    def _parse_clear_cache(self):
        node = ASTNode("ClearCacheStmt")
        start_token = self._current()

        self._advance()  # skip CLEAR
        if self._current().is_keyword("CACHE"):
            self._advance()
    ...
    def _parse_refresh(self):
        node = ASTNode("RefreshStmt")
        start_token = self._current()

        self._advance()  # skip REFRESH

        # TABLE or FUNCTION
        if self._current().is_keyword("TABLE"):
            self._advance()
            node.children["object_type"] = "TABLE"
        elif self._current().is_keyword("FUNCTION"):
            self._advance()
            node.children["object_type"] = "FUNCTION"
    ...
    def _parse_add_jar(self):
        node = ASTNode("AddJarStmt")
        start_token = self._current()

        self._advance()  # skip ADD
        if self._current().is_keyword("JAR"):
            self._advance()
        ...
        while not self._at_end():
            remaining.append(self._advance())
        node.children["jar_path"] = self._tokens_to_text(remaining).strip()
    ...
    def _parse_list_jar(self):
        node = ASTNode("ListJarStmt")
        start_token = self._current()

        self._advance()  # skip LIST
        if self._current().is_keyword("JAR"):
            self._advance()

        # Optional JAR path/filter
        remaining = []
        while not self._at_end():
            remaining.append(self._advance())
        node.children["jar_path"] = self._tokens_to_text(remaining).strip()
...
def parse_sql(sql_text):
    ...
    parser = SparkSQLParser(tokens, raw_sql=sql_text)
    result = parser.parse()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python spark_sql_parser.py <sql_text_or_file>")
        sys.exit(1)
