# -*- coding: utf-8 -*-
"""
MRS Spark SQL Tokenizer

Lexical analyzer for MRS Spark SQL statements.
...
class SparkSQLTokenizer:
    """
    MRS Spark SQL Lexical Analyzer

    Tokenizes SQL text following the Spark SQL lexical rules.
    Supports Spark-specific tokens including:
    - Backtick-quoted identifiers `table_name`
    - Optimizer hints /*+ BROADCAST(...) */, /*+ REPARTITION(...) */, etc.
    - Spark SQL keywords
    """
    ...
    def _read_string(self, quote_char="'"):
        ...
        while self.pos < len(self.text):
            ...
            if ch == quote_char:
                ...
                # Spark allows multi-line string literals (e.g. in OPTIONS, TBLPROPERTIES)
                result.append(ch)
                self._advance()
            else:
                result.append(ch)
    ...
    def _read_number(self):
        ...
        - L suffix for BigInt (e.g., 100L) -- Spark 3.x extension
        """
        start_line = self.line
        start_col = self.column
        start_pos = self.pos
    ...
    def _read_hex_number(self):
        """Read a hexadecimal integer literal like 0x1A, 0XFF."""
        start_line = self.line
        start_col = self.column
        start_pos = self.pos
        # Skip 0x / 0X
    ...
    def tokenize(self):
        ...
        while self.pos < len(self.text):
            ...
            # ---- String constants ----
            # Spark supports both single-quote and double-quote string literals
            if ch == "'" or ch == '"':
            ...
            # Backtick-quoted identifier (Spark/Hive-style)
            if ch == '`':
...
def tokenize(sql_text):
    ...
    Returns:
        tuple: (tokens, errors)
    """
    tokenizer = SparkSQLTokenizer(sql_text)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python spark_sql_tokenizer.py <sql_text_or_file>")
        sys.exit(1)
