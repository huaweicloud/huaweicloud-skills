...
Check the NodeAgent script logs for configuration restore (restore) related records, confirming whether the backed-up configuration files have been correctly restored:
...
- OMM process tree → `get_host_process` (manager mode alternative)
...
| <port> | Component config file | e.g., 20013, 20015 |
