...
def get_project_id_by_region(ak: str, sk: str, region: str):
...
def create_flexus_l_instance(ak, sk, project_id, region, instance_name, flavor="medium", wait=True, timeout=600):
...
def query_instance_status(resource_id):
...
def get_instance_info(resource_id):
    from huaweicloudsdkcore.auth.credentials import GlobalCredentials
    from huaweicloudsdkrms.v1 import RmsClient
    from huaweicloudsdkrms.v1.region.rms_region import RmsRegion
...
def parse_args():
    ...
    parser.add_argument('--region', type=str, default='cn-north-4', help='Region (仅支持 cn-north-4)')
...
def main():
    ...
    if args.region != 'cn-north-4':
        print(f"[ERROR] 仅支持 cn-north-4 区域，当前指定的区域: {args.region}")
        print("[INFO] 请使用 --region cn-north-4 或使用默认值")
        sys.exit(1)
    ...
    project_id = get_project_id_by_region(AK, SK, args.region)
    if not project_id:
        print("[ERROR] Failed to get Project ID")
        sys.exit(1)
    print(f"[INFO] Project ID: {project_id}")

    order_id = create_flexus_l_instance(AK, SK, project_id, args.region, args.name, args.flavor, args.wait, args.timeout)
    if not order_id:
