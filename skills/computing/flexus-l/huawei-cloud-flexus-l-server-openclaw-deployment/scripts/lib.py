...
def get_coc_client(ak: str, sk: str, security_token: str = None):
    ...
    if not ak or not sk:
        raise ValueError("COC credentials not configured, please provide AK and SK parameters. For temporary credentials, also provide security_token.")
...
def create_openclaw_instance(instance_name = None, region = None, ak = None, sk = None, security_token = None):
    ...
    Args:
        ...
        ak: Huawei Cloud AK (supports both long-term and temporary AK), required
        sk: Huawei Cloud SK (supports both long-term and temporary SK), required
        security_token: Security token for temporary credentials (optional, only required for temporary AK/SK)
    ...
    if not ak or not sk:
        return {
            ...
            "error": {
                "code": "CONFIG_ERROR",
                "message": "Please provide AK and SK parameters. For temporary credentials, also provide security_token."
            }
        }
