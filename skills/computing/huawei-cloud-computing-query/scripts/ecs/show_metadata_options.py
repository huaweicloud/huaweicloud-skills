...
from huaweicloudsdkecs.v2.model import ShowMetadataOptionsRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询 ECS 服务器元数据选项")
...
try:
    ...
    request = ShowMetadataOptionsRequest()
    request.server_id = args.server_id
    response = client.show_metadata_options(request)

    # 渲染结果
    output = ""
    output += f"http_endpoint: {getattr(response, 'http_endpoint', '')}\n"
    output += f"http_tokens: {getattr(response, 'http_tokens', '')}\n"
    print(output)
except Exception as e:
    print(f"ecs.metadata_options 查询失败: {e}")
    exit(1)
