...
from huaweicloudsdkecs.v2.model import ShowSerialConsoleActionsRequest
...
# 参数
parser = argparse.ArgumentParser(description="查询 ECS 服务器串口控制台")
...
try:
    ...
    request = ShowSerialConsoleActionsRequest()
    request.server_id = args.server_id
    response = client.show_serial_console_actions(request)
    serial_console = response.serial_console

    if not serial_console:
        print(f"没有找到 ECS 服务器串口控制台 (区域: {Region}, 服务器 ID: {args.server_id})")
        exit(0)

    # 渲染结果
    output = ""
    output += f"url: {getattr(serial_console, 'url', '')}\n"
    print(output)
except Exception as e:
    print(f"ecs.serial_console_actions 查询失败: {e}")
    exit(1)
