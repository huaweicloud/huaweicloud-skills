#!/usr/bin/env python3
"""skill-quality-cli — Skill 质量上报 CLI (内置源码本地部署版)。
...
实现原则:
  ...
  - session 三渠道: --session-id > SKILL_QUALITY_SESSION_ID > qcfg 逐级回退
  - 手动关闭: SKILL_QUALITY_REPORT=0 时 run 正常执行但不上报(opt-out)
  - 安全约束(SC2 供应链): 不包含任何运行时外部下载/版本拉取逻辑; install/upgrade/
    bootstrap 仅在本地部署本仓库内置源码, 版本固定为内置 CLI_VERSION。
  - 安全约束(TM1 工具参数): run 包装的子进程命令须通过白名单+类型校验, 非法输入拒绝。
"""
...
# skill-quality-cli — 透明 wrapper: 执行内置 Python 源码(本地部署, 无外部下载)
SELF_DIR="$(cd "$(dirname "$0")" && pwd)"

# 定位 cli_entry.py
ENTRY=""
if [ -f "${SELF_DIR}/skill-quality-cli.d/cli_entry.py" ]; then
    ENTRY="${SELF_DIR}/skill-quality-cli.d/cli_entry.py"
elif [ -f "${SELF_DIR}/../scripts/cli/cli_entry.py" ]; then
    ENTRY="${SELF_DIR}/../scripts/cli/cli_entry.py"
fi
if [ -z "$ENTRY" ]; then
    echo "skill-quality-cli: 找不到 cli_entry.py, 请重新安装" >&2
    exit 1
fi
...
def _deploy_wrapper_and_fallback():
    """本地部署: 在本仓库内置源码目录旁生成 wrapper, 并把自身与 cli_reporting.py
    复制为 fallback 源码。全程无网络, 无外部代码拉取。"""
    ...
    _maybe_reporting = os.path.join(_self_dir, "cli_reporting.py")
    if os.path.isfile(_maybe_reporting):
        _shutil.copy2(_maybe_reporting, os.path.join(FALLBACK_DIR, "cli_reporting.py"))
...
def cmd_install(args):
    """本地部署内置源码(wrapper + Python fallback), 无任何外部下载。"""
...
def cmd_upgrade(args):
    if not os.path.isfile(VERSION_FILE):
        ...
        sys.exit(1)
    print(f"skill-quality-cli 当前为本地内置版本 {CLI_VERSION}, 无外部升级通道")
    _deploy_wrapper_and_fallback()
    _write_version_file(CLI_VERSION, WRAPPER_TARGET)
    print("已重新本地部署内置源码(实际上限即内置版本)")


def cmd_bootstrap(args):
    """零依赖冷启动安装: 直接本地部署内置源码, 不联网。"""
    _deploy_wrapper_and_fallback()
    _write_version_file(CLI_VERSION, WRAPPER_TARGET)
...
def _resolve_session_id(cli_session_id, qcfg):
    """session_id 三渠道(v1.1.8): --session-id 参数 > SKILL_QUALITY_SESSION_ID env > qcfg。
    返回 None 时由 cli_reporting.report() 兜底(宿主会话库轻取 session 标识, 零内容)。"""
    if cli_session_id:
        return cli_session_id
    _env_sid = os.environ.get("SKILL_QUALITY_SESSION_ID") or ""
    if _env_sid:
        return _env_sid
    return qcfg.get("session_id") or None
...
def cmd_report(args):
    ...
    # v1.1.8: SKILL_QUALITY_REPORT=0 手动关闭上报(opt-out)
    if os.environ.get("SKILL_QUALITY_REPORT") == "0":
        return ""
...
def main(argv=None):
    ...
    parser.add_argument("--no-auto-upgrade", action="store_true", help="禁用自动升级到最新版(本地内置版默认不升级)")
    sub = parser.add_subparsers(dest="cmd", required=True)
    sub.add_parser("install", help="本地部署内置版 CLI")
    sub.add_parser("upgrade", help="重新本地部署内置版 CLI")
    sub.add_parser("bootstrap", help="零依赖冷启动安装(内置源码本地部署)")
    ...
    p_run.add_argument("--session-id", default=None, help="Agent 会话ID(三渠道: --session-id > SKILL_QUALITY_SESSION_ID > qcfg)")
    ...
    p_report.add_argument("--session-id", default=None, help="Agent 会话ID(三渠道: --session-id > SKILL_QUALITY_SESSION_ID > qcfg)")
