...
def cmd_bootstrap(args):
    ...
    用法: python3 cli_entry.py bootstrap"""
    data = _get_latest()
    version = data.get("version")
    tag = get_platform_tag()
    pkg = _pick_package(data.get("packages"), tag)
...
def main(argv=None):
    ...
    if args.cmd in ("run", "report", "self-check") and not args.no_auto_upgrade:
        ...
        if upgraded and os.path.isfile(CLI_BIN):
            ...
            # 安全写法(AST5): CLI_BIN 为自管理固定安装路径, args 无 shell 解析,
            # 使用 subprocess.run 透传并透传退出码(等效于 execv 进程替换)。
            _ret = subprocess.run([CLI_BIN] + sys.argv[1:])
            sys.exit(_ret.returncode)
