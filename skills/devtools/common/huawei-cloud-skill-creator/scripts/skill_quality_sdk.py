...
def report(
    *,
    ...
    """手动上报一次 Skill 执行质量。上报成功(HTTP 200)时返回 trace_id；
    上报未发出(禁用/无凭证/端点校验失败/网络错误/非200响应)时返回空字符串 ""，
    调用方不得据此输出"上报成功"。
    ...
    if not _post(payload, json_creds=json_creds):
        # v2.13.1: 上报未发出时返回空串(而非 trace_id), 杜绝"假成功" —
        # 禁用/SSRF拦截/无凭证/网络失败/非200响应场景下调用方必须能识别未上报。
        logger.warning(
            "skill quality report NOT delivered (disabled/no credentials/endpoint rejected/network error), trace_id=%s, skill_name=%s",
            trace_id, skill_name or "",
        )
        return ""
    return trace_id
...
def _load_json_config():
    ...
    文件存在但 JSON 非法时置 _QC_CONFIG_INVALID=True 并返回 {}（调用方需据此如实上报失败）。"""
    global _QC_CONFIG_INVALID
    cfg = {}
    d = os.getcwd()
    for _ in range(4):
        ...
        if os.path.isfile(p):
            try:
                ...
                _QC_CONFIG_INVALID = True
                logger.error("读取 .quality_report.json 失败(非法 JSON): %s", e)
            break
...
def cli_report(args):
    ...
    # 非法 JSON 配置: 会话/归属上下文不可信, 如实上报失败而非谎报成功。
    if _QC_CONFIG_INVALID:
        print("上报失败: .quality_report.json 非法 JSON, 会话上下文不可信(未发送)", file=sys.stderr)
        return 1
    ...
    if trace_id:
        ...
        return 0
    # v2.13.1: 上报未发出必须显式失败输出(非"上报成功"), CLI 以非0退出。
    print(
        f"上报未发出: 无可用凭证/未配置游客通道/端点校验失败或网络错误, skill_name={args.skill_name or cfg.get('skill_name')}",
        file=sys.stderr,
    )
    return 1
