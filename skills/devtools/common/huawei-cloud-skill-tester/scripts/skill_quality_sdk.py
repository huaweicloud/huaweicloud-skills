...
配置(环境变量, 均可选):
    ...
    SKILL_QUALITY_AK         华为云 AK (回退 HUAWEI_ACCESS_KEY / HUAWEICLOUD_SDK_AK / HUAWEI_CLOUD_SDK_AK / HW_ACCESS_KEY / HWC_ACCESS_KEY)
    SKILL_QUALITY_SK         华为云 SK (回退 HUAWEI_SECRET_KEY / HUAWEICLOUD_SDK_SK / HUAWEI_CLOUD_SDK_SK / HW_SECRET_KEY / HWC_SECRET_KEY)
    SKILL_QUALITY_INSECURE   设为 1 跳过 SSL 证书验证(自定义域名未配证书时用)
    SKILL_QUALITY_NAME       skill 名称(report() 未显式传 skill_name 时读取该环境变量)
    ...
    SKILL_QUALITY_ALLOW_ANONYMOUS  设为 1(默认)时允许无凭证降级匿名上报:
                                   无 AK/SK 无法获取 IAM Token 时直连 APIG 公网接口,
                                   payload 标注 unauthenticated=1, 保证 C01(凭证缺失)等
                                   sys_fail 事件仍可计入运营台; 设为 0 则沿用旧行为(不上报)

鉴权: 通过 AK/SK 获取华为云 IAM Token, 带 X-Auth-Token 请求 APIG 公网接口。
      无 AK/SK 时若 SKILL_QUALITY_ALLOW_ANONYMOUS=1 则降级匿名直连上报
      (payload 标注 unauthenticated=1, 不携带任何凭证), 否则不上报。
      Token 缓存至 expires_at 自动刷新。
...
def _skill_name() -> str:
    """report()/装饰器未显式传 skill_name 时回落的 skill 名称(SKILL_QUALITY_NAME)。"""
    return os.environ.get("SKILL_QUALITY_NAME", "")


def _allow_anonymous() -> bool:
    """无凭证降级匿名上报开关(SKILL_QUALITY_ALLOW_ANONYMOUS, 默认 1)。

    无 AK/SK 无法取 IAM Token 时直连 APIG 公网接口, payload 标注
    unauthenticated=1, 保证 C01(凭证缺失)等 sys_fail 事件仍可上报。
    """
    return os.environ.get("SKILL_QUALITY_ALLOW_ANONYMOUS", "1") == "1"
...
def _read_ak_sk():
    ...
    回退链与 skill 自身认证链路(lib/utils.sh)一致:
    HUAWEI_ACCESS_KEY / HUAWEI_SECRET_KEY 是 tester 的标准凭证变量
    (utils.sh 将所有 HUAWEI*/HW*/HWC* 前缀的 AK/SK 变量归一化为这两个),
    必须优先读取, 否则仅配置 hcloud CLI 凭证的场景下上报会静默丢失。
    同时保留 SKILL_QUALITY_AK/SK 及华为云 SDK 标准变量回退。
    """
    ak = (os.environ.get("SKILL_QUALITY_AK")
          ...
          or os.environ.get("HWC_ACCESS_KEY"))
    sk = (os.environ.get("SKILL_QUALITY_SK")
          ...
          or os.environ.get("HWC_SECRET_KEY"))
    return ak, sk
...
def _post(payload: dict) -> bool:
    ...
    优先带 IAM Token 上报; 无凭证(无 AK/SK / Token 获取失败)时,
    若 SKILL_QUALITY_ALLOW_ANONYMOUS=1(默认)则降级匿名直连 APIG 公网接口,
    payload 标注 unauthenticated=1 —— 保证 C01(凭证缺失)等 sys_fail 事件结构上可达,
    否则这类事件(如 exit 77)永远无法计入运营台。
    """
    if DISABLED:
        return False
    ...
    if not token:
        if not _allow_anonymous():
            logger.debug("无 IAM Token 且 SKILL_QUALITY_ALLOW_ANONYMOUS=0, 跳过上报")
            return False
        logger.debug("无 IAM Token, 降级匿名直连上报 (SKILL_QUALITY_ALLOW_ANONYMOUS=1)")
        payload = dict(payload)
        payload["unauthenticated"] = 1
...
def report(
    *,
    ...
    payload = {
        ...
        "skill_name": skill_name or _skill_name() or None,
...
class quality_context:
    ...
    def __init__(self, skill_name=None, *, skill_id=None, skill_version=None,
        ...
        self.skill_name = skill_name or _skill_name() or None
...
def quality_report(skill_name=None, *, skill_id=None, skill_version=None,
    ...
    def decorator(fn):
        ...
        def wrapper(*args, **kwargs):
            name = skill_name or _skill_name() or fn.__name__
...
def self_check():
    ...
    if not ak or not sk:
        if _allow_anonymous():
            print("无 AK/SK, 使用匿名降级上报 (SKILL_QUALITY_ALLOW_ANONYMOUS=1)")
        else:
            print("无 AK/SK 且 SKILL_QUALITY_ALLOW_ANONYMOUS=0, 上报不可用")
            return
    trace_id = report(
