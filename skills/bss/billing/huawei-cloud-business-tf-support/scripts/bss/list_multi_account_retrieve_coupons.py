...
from huaweicloudsdkbss.v2.model import ListMultiAccountRetrieveCouponsRequest
from huaweicloudsdkbss.v2.region.bss_region import BssRegion

AK, SK, Region, SecurityToken = load_credentials()

parser = argparse.ArgumentParser(description="查询企业多账户可回收代金券")
...
parser.add_argument("--limit", type=int, default=50, help="每次查询条数。默认值为50")
...
try:
    request = ListMultiAccountRetrieveCouponsRequest()
    ...
    response = client.list_multi_account_retrieve_coupons(request)

    total_count = getattr(response, 'total_count', 0)
    avail_retrieve_coupons = getattr(response, 'avail_retrieve_coupons', []) or []

    output = f"总记录数\t{total_count}\n"
    
    if avail_retrieve_coupons:
        output += "\n可回收代金券列表:\n"
        output += "主优惠券ID\t促销计划名称\t子优惠券ID\t优惠券余额\t生效时间\t失效时间\n"
        for coupon in avail_retrieve_coupons:
            coupon_id = getattr(coupon, 'coupon_id', '')
            plan_name = getattr(coupon, 'plan_name', '')
            sub_coupon_id = getattr(coupon, 'sub_coupon_id', '')
            balance = getattr(coupon, 'balance', '')
            effective_time = getattr(coupon, 'effective_time', '')
            expire_time = getattr(coupon, 'expire_time', '')
            output += f"{coupon_id}\t{plan_name}\t{sub_coupon_id}\t{balance}\t{effective_time}\t{expire_time}\n"
    ...
    print(f"bss.list_multi_account_retrieve_coupons 查询失败: {e}")
    exit(1)
