...
from huaweicloudsdkvpn.v5.model import ListVpnGatewayJobsRequest
...
parser = argparse.ArgumentParser(description="查询 VPN 网关任务列表")
parser.add_argument("--project_id", type=str, required=True, help="项目 ID，可通过 ../iam/get_project_id.py 获取")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--resource_id", type=str, required=True, help="资源 ID，VPN 网关 ID，可通过 list_vgws.py 获取")
...
try:
    ...
    request = ListVpnGatewayJobsRequest()
    request.resource_id = args.resource_id
    response = client.list_vpn_gateway_jobs(request)
    items = response.jobs

    if not items:
        print(f"没有找到 VPN 网关任务 (区域: {Region})")
        exit(0)
    ...
    print(f"vpn.list_vpn_gateway_jobs 查询失败: {e}")
    exit(1)
