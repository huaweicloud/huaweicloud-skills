...
from huaweicloudsdkvpn.v5.model import ListVpnConnectionsRequest
...
parser = argparse.ArgumentParser(description="查询 VPN 连接列表")
parser.add_argument("--project_id", type=str, required=True, help="项目 ID，可通过 ../iam/get_project_id.py 获取")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--vgw_id", type=str, help="VPN 网关 ID，可通过 list_vgws.py 获取")
parser.add_argument("--vpn_id", type=str, help="VPN 连接 ID")
parser.add_argument("--eip_id", type=str, help="弹性公网 IP ID")
parser.add_argument("--vgw_ip", type=str, help="VPN 网关 IP")
parser.add_argument("--enterprise_project_id", type=str, help="企业项目 ID，多个以逗号分隔，可通过 ../eps/list_enterprise_projects.py 获取")
parser.add_argument("--name", type=str, help="VPN 连接名称，用于从结果中按名称模糊过滤（自定义过滤，需全量拉取）")
...
def render(items, total_count=None, has_more=False, next_marker=None):
    ...
    output = f"id\tname\tstatus\tvgw_id\tvgw_ip\tcgw_id\tstyle\tpeer_subnets\tenterprise_project_id\tcreated_at\tupdated_at\n"
    for item in items:
        id = getattr(item, 'id', '')
        ...
        output += f"{id}\t{name}\t{status}\t{vgw_id}\t{vgw_ip}\t{cgw_id}\t{style}\t{', '.join(peer_subnets)}\t{enterprise_project_id}\t{created_at}\t{updated_at}\n"
    ...
    if total_count is not None:
        ...
        if has_more and next_marker:
            output += f"\n可使用 --marker={next_marker} 继续查询下一页，或使用 --vgw_id / --vpn_id / --name 等参数缩小查询范围"
    elif has_more:
...
try:
    ...
    has_custom_filter = args.name is not None
    ...
    if has_custom_filter:
        ...
        while True:
            ...
            if args.vpn_id:
                request.vpn_id = args.vpn_id
            if args.eip_id:
                request.eip_id = args.eip_id
            if args.vgw_ip:
                request.vgw_ip = args.vgw_ip
            ...
            items = response.vpn_connections
            if not items:
                break
        ...
        if not all_items:
            print(f"没有找到 VPN 连接 (区域: {Region})")
            exit(0)

        # 本地过滤
        filtered_items = [item for item in all_items if args.name in getattr(item, 'name', '')]

        if not filtered_items:
            print(f"没有找到 VPN 连接 (区域: {Region}, 名称过滤: {args.name})")
            exit(0)
    ...
    print(f"vpn.list_vpn_connections 查询失败: {e}")
    exit(1)
