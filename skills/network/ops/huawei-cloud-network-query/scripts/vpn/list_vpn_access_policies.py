...
from huaweicloudsdkvpn.v5.model import ListVpnAccessPoliciesRequest
...
parser = argparse.ArgumentParser(description="查询 VPN 访问策略列表")
...
def render(items, total_count=None, has_more=False, next_marker=None):
    """
    渲染 VPN 访问策略列表
    :param items: 策略列表（最多 PAGE_SIZE 条）
...
try:
    ...
    # 只查1次
    request = ListVpnAccessPoliciesRequest()
    request.vpn_server_id = args.vpn_server_id
    request.limit = FETCH_SIZE
    if args.marker:
        request.marker = args.marker

    response = client.list_vpn_access_policies(request)
    items = response.access_policies

    if not items:
        print(f"没有找到 VPN 访问策略 (区域: {Region}, VPN 服务端 ID: {args.vpn_server_id})")
        exit(0)
    ...
    print(f"vpn.list_vpn_access_policies 查询失败: {e}")
    exit(1)
