...
from huaweicloudsdkvpn.v5.model import ListVpnUsersRequest
...
parser = argparse.ArgumentParser(description="查询 VPN 用户列表")
...
def render(items, total_count=None, has_more=False, next_marker=None):
    ...
    output = f"id\tname\tdescription\tuser_group_id\tuser_group_name\tstatic_ip\tcreated_at\tupdated_at\n"
    for item in items:
        id = getattr(item, 'id', '')
...
try:
    ...
    # 只查1次
    request = ListVpnUsersRequest()
    request.vpn_server_id = args.vpn_server_id
    request.limit = FETCH_SIZE
    if args.marker:
        request.marker = args.marker

    response = client.list_vpn_users(request)
    items = response.users

    if not items:
        print(f"没有找到 VPN 用户 (区域: {Region}, VPN 服务端 ID: {args.vpn_server_id})")
        exit(0)
    ...
    print(f"vpn.list_vpn_users 查询失败: {e}")
    exit(1)
