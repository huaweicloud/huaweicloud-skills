...
from huaweicloudsdkvpn.v5.model import ListP2cVpnGatewayJobsRequest
...
parser = argparse.ArgumentParser(description="查询 P2C VPN 网关任务列表")
parser.add_argument("--project_id", type=str, required=True, help="项目 ID，可通过 ../iam/get_project_id.py 获取")
parser.add_argument("--region", type=str, help="区域，默认 cn-north-4")
parser.add_argument("--resource_id", type=str, required=True, help="资源 ID，P2C VPN 网关 ID，可通过 list_p2c_vgws.py 获取")
...
def render(items, total_count=None, has_more=False, next_marker=None):
    """
    渲染 P2C VPN 网关任务列表
    :param items: 任务列表（最多 PAGE_SIZE 条）
    :param total_count: 总数，None 表示未知
    :param has_more: 是否还有更多数据
    :param next_marker: 下一页的 marker
    """
    if not items:
        print("没有找到 P2C VPN 网关任务")
        return

    output = f"id\tresource_id\tjob_type\tstatus\tcreated_at\tfinished_at\terror_message\n"
    for item in items:
        id = getattr(item, 'id', '')
        ...
        finished_at = getattr(item, 'finished_at', '')
...
try:
    ...
    request = ListP2cVpnGatewayJobsRequest()
    request.resource_id = args.resource_id
    response = client.list_p2c_vpn_gateway_jobs(request)
    items = response.jobs

    if not items:
        print(f"没有找到 P2C VPN 网关任务 (区域: {Region})")
        exit(0)
    ...
    print(f"vpn.list_p2c_vpn_gateway_jobs 查询失败: {e}")
    exit(1)
